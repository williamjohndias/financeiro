...
export const calcularSaldoMensal = (
  ...
  // Contabilizar APENAS os gastos do cartão PAGOS do mês
  // Funciona como espelho do banco - só conta quando realmente pago
  const gastosCartaoFiltrados = gastosCartao.filter(g => g.mes === mes);
  const gastosCartaoMes = gastosCartaoFiltrados
    .reduce((sum, g) => {
      if (g.pago) {
        // Se pago, usar valorPago se existir (para abatimento parcial), senão usar valorParcela completo
